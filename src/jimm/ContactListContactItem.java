...
public class ContactListContactItem extends ContactListItem implements CommandListener {
    ...
    public boolean returnBoolValue(int value) {
        switch (value) {
            ...
            case VALUE_NO_AUTH:
                return (noAuth);
            case VALUE_CHAT_SHOWN:
                return Jimm.jimm.getChatHistoryRef().chatHistoryShown(this.uin);
            case VALUE_IS_TEMP:
                return (this.temporary);
            case VALUE_HAS_CHAT:
                return Jimm.jimm.getChatHistoryRef().chatHistoryExists(this.uin);
            default:
                return false;
        }
    }
    ...
    public void setBoolValue(int value, boolean bool_value) {
        switch (value) {
            case VALUE_ADDED:
                this.added = bool_value;
                break;
            case VALUE_NO_AUTH:
                this.noAuth = bool_value;
                break;
            case VALUE_IS_TEMP:
                this.temporary = bool_value;
        }
    }
    ...
    public void commandAction(Command c, Displayable d) {
        ...
        // Textbox has been closed
            ...
            // Reason has been entered
                ...
                try {
                    ...
                    if (ContactListContactItem.this.returnBoolValue(VALUE_IS_TEMP))
                        Jimm.jimm.getIcqRef().requestAction(updateAct);
                } catch (JimmException e) {
                    JimmException.handleException(e);
                    if (e.isCritical()) return;
                }
        ...
        // User select a emotion
            ...
            messageTextbox.insert(" " + Jimm.jimm.getEmotionsRef().getSelectedEmotion() + " ", caretPos);
        }
